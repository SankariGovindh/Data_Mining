...
with open(file_path, mode='r') as infile:
    ...
    dataDict = [(rows[0]+"-"+rows[1], rows[5].lstrip("0")) for rows in reader] #(productID-customerID, 4567), etc...
...
groceryRDD = dataRDD.map(lambda x: (x[0],[x[1]]))\
                     .reduceByKey(lambda a,b:a+b).map(lambda x:(x[0],set(x[1]))).mapValues(list)\
                     .filter(lambda x:len(x[1]) > filterThreshold).map(lambda p:p[1])
...
for candidate in candidateList:
    ...
    # singleton candidate
    if (len(candidate) == 1):
        candidateOp = candidateOp + "(" + "'" + candidate[0] + "'" + ")" + ","

    # greater size
    else:
        candidateOp = candidateOp + str(candidate)

# writing data to file - frequent items
frequentOp = ""
for frequent in frequentItem:

    # singleton candidate
    if (len(frequent) == 1):
        tempStr = ''.join(frequent[0])

        for word in tempStr.split():
            if word.isdigit():
                frequentOp = frequentOp + "(" + "'" + word + "'" + ")" + ","

                # greater size
    else:
        frequentOp = frequentOp + str(frequent)
